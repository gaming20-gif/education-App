...
export default function Navbar({
  ...
  return (
    <>
      ...
      {/* ------------------------------------------------------------- */}
      <header className="sticky top-0 z-40 bg-[#1E40AF] text-white shadow-md border-b border-blue-900/30 px-3 sm:px-6 py-2">
        <div className="max-w-7xl mx-auto flex items-center justify-between gap-2">
          
          {/* Logo & Portal Badge */}
          <div 
            onClick={onResetHome}
            className="flex items-center gap-2 cursor-pointer group select-none active:scale-[0.97] transition-transform"
          >
            <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-xl bg-white/10 border border-white/20 flex items-center justify-center shadow-md group-hover:bg-white/20 transition-all flex-shrink-0">
              <GraduationCap className="w-4 h-4 sm:w-5 sm:h-5 text-amber-400" />
            </div>
            <div className="flex items-center gap-1.5">
              <span className="font-bold text-base sm:text-lg tracking-tight text-white group-hover:text-amber-200 transition-colors">
                EduNexus
              </span>
              <span className="text-[9px] sm:text-[10px] font-semibold px-1.5 py-0.5 rounded-full bg-amber-500/20 text-amber-300 border border-amber-400/30 whitespace-nowrap">
                Academic Portal
              </span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-1.5 flex-shrink-0">
            <button
              onClick={onResetHome}
              className="flex items-center gap-1 px-2.5 py-1 rounded-xl bg-blue-800/80 hover:bg-blue-900 text-white text-xs font-bold border border-blue-700 transition-colors shadow-sm active:scale-[0.97]"
            ...
            {currentUser ? (
              <div className="flex items-center gap-1.5">
                <button
                  onClick={onOpenLogin}
                  className="flex items-center gap-1 px-2.5 py-1 rounded-xl bg-amber-400 hover:bg-amber-300 text-blue-950 font-bold text-xs shadow-md transition-all active:scale-[0.97]"
                  title="Enrolment Profile"
                >
                  <div className="w-4 h-4 rounded-full bg-blue-900 text-amber-300 text-[10px] font-extrabold flex items-center justify-center flex-shrink-0">
                    {currentUser.fullName ? currentUser.fullName.charAt(0) : "S"}
                  </div>
                  <span className="truncate max-w-[65px] sm:max-w-[120px] text-[11px]">{currentUser.fullName}</span>
                </button>

                <button
                  onClick={onLogout}
                  className="p-1 sm:px-2.5 sm:py-1 rounded-xl bg-red-600 hover:bg-red-700 text-white font-bold text-xs shadow-md border border-red-500 transition-all active:scale-[0.97]"
                  title="Log Out"
                >
              ...
              <button
                onClick={onOpenLogin}
                className="flex items-center gap-1 px-3 py-1 rounded-xl bg-amber-500 hover:bg-amber-400 text-blue-950 font-bold text-xs shadow-md transition-all active:scale-[0.97]"
              >
